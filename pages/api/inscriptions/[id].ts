...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    console.log('Get inscription request headers:', req.headers);
    const { id } = req.query;
    if (!id || typeof id !== 'string') {
      return res.status(400).json({ message: 'Missing or invalid inscription_id' });
    }
    ...
    console.log('Get inscription response:', response.data);
    res.status(200).json(response.data);
  } catch (error: any) {
    console.error('Skrybit API error (get inscription):', {
